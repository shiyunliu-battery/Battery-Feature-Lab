...
def convert_with_bds(
    ...
    # ConversionReport intentionally remains the provider-owned object and is
    # still serialized by its own writer. Replace only host-local filesystem
    # locations with portable artifact references before public serialization.
    report.input_path = artifact_reference(input_path)
    report.output_path = artifact_reference(normalized_path)
