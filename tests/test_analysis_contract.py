...
def test_analyze_writes_index_metadata_evidence_and_calls_pyprobe(
    ...
    input_report = json.loads(result.input_report_path.read_text(encoding="utf-8"))
    for payload in (results, evidence, metadata, validation, input_report):
        _assert_portable_json(payload)
    assert evidence["source"]["input_path"] == vendor_csv.name
    assert evidence["configuration"]["output_dir"] == "."
    assert metadata["dataset"]["source_file"] == vendor_csv.name
    assert input_report["input_path"] == vendor_csv.name
    assert input_report["output_path"] == result.normalized_data_path.name
    ...
    absolute_evidence = json.loads(json.dumps(evidence))
    absolute_evidence["source"]["input_path"] = r"C:\Users\person\cell.csv"
    assert validate_payload(absolute_evidence, RESULTS_SCHEMA)
    absolute_metadata = json.loads(json.dumps(metadata))
    absolute_metadata["dataset"]["source_file"] = "/home/person/cell.csv"
    assert validate_payload(absolute_metadata, METADATA_SCHEMA)
...
def test_bds_report_is_direct_serialization(vendor_csv: Path, tmp_path: Path, monkeypatch) -> None:
    ...
    assert on_disk["input_path"] == vendor_csv.name
    assert on_disk["output_path"] == result.normalized_data_path.name
...
def test_run_id_is_independent_of_output_location(tmp_path: Path) -> None:
    ...
    assert first_evidence["configuration"]["output_dir"] == "."
    assert second_evidence["configuration"]["output_dir"] == "."
